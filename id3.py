...
def buscar_maior_ganho(caminhos_entropia):
    ...
    for i in caminhos_entropia:
        if caminhos_entropia[i]['ganho'] > maior_ganho:
            maior_ganho = caminhos_entropia[i]['ganho']
...
with open('saida_id3.json', 'w') as json_file:
    json.dump(id3(df), json_file, indent=4)